...
Before training the model, you must prepare the raw data:

1. **Upload Data:** Place your raw dataset file into the `data/raw/` directory.
2. **Preprocessing:** Run the preprocessing script to clean the text (e.g., remove URLs, special characters, and noise). This ensures the data is in the correct format for training:
```bash
...
The cleaned dataset will be saved automatically to `data/processed/`.
